...
const ToolBox: FC<ToolBoxProps> = ({ tool }) => {
  const imageProps = tool?.image
    ...
    : { src: "/hey-rebekah-logo.svg", alt: `${tool.name ? tool.name : "Hey Rebekah Built With Tool"} Logo`, fill: true, className: "object-contain" };
  ...
  const categoryColor = color[tool?.category?.color as keyof typeof color] || "bg-white";
  ...
  return (
    <>
      <div className="group relative">
        ...
        >
          {imageProps && <Image {...imageProps} alt={`${tool.name ? tool.name : "Hey Rebekah Built With Tool"} Logo`} className="object-contain p-2" />}
