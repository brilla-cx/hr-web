...
const getAuthorPost = cache(async (slug) => {
  const posts = await getPaginatedAuthorsPosts(slug);
  return posts;
});

export default async function AuthorPaginatedPosts({ slug, searchParams }) {
  ...
  const posts = await getAuthorPost(params);
